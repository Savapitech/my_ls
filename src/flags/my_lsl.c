...
void print_ls_buff(ls_buff_t *ls_buff)
{
    for (int i = 0; ls_buff[i].name.str; i++) {
        my_printf("%d %s %s %d %.16s %s\n", ls_buff[i].inodes,
            ls_buff[i].user.str, ls_buff[i].group.str, ls_buff[i].size,
            ls_buff[i].date.str, ls_buff[i].name.str);
    }
}
    ...
    lsinfo_t *lsinfo, int i)
{
    ...
    // my_rights_calc(&st);
    ls_buff[i].inodes = st.st_nlink;
    ls_buff[i].user.str = passwd->pw_name;
    ls_buff[i].group.str = grp->gr_name;
    ls_buff[i].size = st.st_size;
